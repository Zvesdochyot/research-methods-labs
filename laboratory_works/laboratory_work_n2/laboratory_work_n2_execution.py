...
x2 = [-1, -1, 1]
y_min, y_max = 60, 160
x1_min, x1_min_normalized = -25, -1
x1_max, x1_max_normalized = 75, 1
x2_min, x2_min_normalized = -20, -1
x2_max, x2_max_normalized = 60, 1
...
y_matrix = [[random.randint(y_min, y_max) for _ in range(m)] for _ in range(3)]

y_average_value = [sum(y_matrix[i][j] for j in range(m)) / m for i in range(3)]

sigma = [sum([(element - y_average_value[i]) ** 2 for element in y_matrix[i]]) / m for i in range(3)]

sigma_theta = math.sqrt((2 * (2 * m - 2)) / (m * (m - 4)))
...
average_execution_time = timeit.timeit(stmt=code, setup=setup, number=100) / 100
print(f'Середній час виконання програми (в мілісекундах): {average_execution_time * 1000}')
